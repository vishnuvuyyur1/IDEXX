...
public class ResponseMapper {
	...
	public List<ApiResponse> mapFromBookResponse(BookResponse bookResp){
		...
		if(bookResp.getItems()!=null) {
			return bookResp.getItems().stream()
					.map(book -> new ApiResponse(book.getVolumeInfo().getTitle(),book.getVolumeInfo().getAuthors(),Type.BOOK.toString()))
	...
	public List<ApiResponse> mapFromAlbumResponse(AlbumResponse albumResp){
		...
		if(albumResp.getResults()!=null) {
			return albumResp.getResults().stream()
					.map(album -> new ApiResponse(album.getWrapperType(),Arrays.asList(album.getArtistName()),Type.ALBUM.toString()))
					 .collect(Collectors.toList());	
		}
		return new ArrayList<>();
	}
